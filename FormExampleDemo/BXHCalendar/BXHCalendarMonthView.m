...
@property (nonatomic, assign)BOOL isShowWeekend;///<是否显示周末
...
- (instancetype)initWithCalendarLayout:(BXHCalendarLayout *)layout isShowWeekend:(BOOL)isShowWeekend
{
    if (self = [super init])
    {
        _isShowWeekend = isShowWeekend;
        _layout = layout;
        _layout.monthView = self;
        _layout.isShowWeekend = YES;
...
- (void)showWeekend:(BOOL)showWeekend{
    ...
    CGFloat itemwidth = showWeekend?ScreenW/9:ScreenW/7;
    self.layout.itemSize = CGSizeMake(itemwidth, itemwidth*CalendarDayView_HW_Ration);
    self.layout.isShowWeekend = showWeekend;
    self.weekLine = 0;
    [self fetchShowDayView];
    [self reload];
}
...
- (void)fetchShowDayView
{
    ...
    else
    {
        [self.dayViewMap enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, BXHCalendarDayView * _Nonnull obj, BOOL * _Nonnull stop) {
            ...
            if (show)
            {
                if (self.isShowWeekend) {
                    [self->_showDaysAry addObject:obj];
                }else{
                    if (obj.indexPath.row==0||obj.indexPath.row==6) {
...
- (void)prepareLayout
{
    ...
    for (BXHCalendarDayView *dayView in _showDaysAry)
    {
        ...
        {
//            NSLog(@"row===%ld",(long)dayView.indexPath.row);
