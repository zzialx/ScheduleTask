...
- (void)showWeekend:(BOOL)showWeekend{
    self.isShowWeekend = showWeekend;
    if (showWeekend) {
        [_weekendView  updateShowWeekend:YES withTemWidth:ITEM_WIDTH];
        _weekendView.frame = CGRectMake(ITEM_WIDTH, _weekendView.frame.origin.y, ScreenW-2*ITEM_WIDTH, _weekendView.frame.size.height);
        ...
        [self.midMonthView showWeekend:YES];
        [self.preMonthView showWeekend:YES];
        [self.nextMonthView showWeekend:YES];
    }else{
        [_weekendView  updateShowWeekend:NO withTemWidth:ITEM_WIDTH5];
        _weekendView.frame = CGRectMake(ITEM_WIDTH5, _weekendView.frame.origin.y, ScreenW-2*ITEM_WIDTH5, _weekendView.frame.size.height);
        _lastWeekBtn.frame = CGRectMake(0, 0, ITEM_WIDTH5, self.bounds.size.height);
        _nextWeekBtn.frame = CGRectMake(self.bounds.size.width-ITEM_WIDTH5, 0, ITEM_WIDTH5, self.bounds.size.height);
        _midMonthView.frame = CGRectMake(ITEM_WIDTH5, CGRectGetMaxY(self.weekendView.frame), self.bounds.size.width-2*ITEM_WIDTH5, CALENDARVIEW_H);
        _preMonthView.frame = CGRectMake(-self.bounds.size.width, CGRectGetMaxY(self.weekendView.frame), self.bounds.size.width-2*ITEM_WIDTH5, CALENDARVIEW_H);
        _nextMonthView.frame = CGRectMake(-self.bounds.size.width, CGRectGetMaxY(self.weekendView.frame), self.bounds.size.width-2*ITEM_WIDTH5, CALENDARVIEW_H);

        [self.midMonthView showWeekend:NO];
        [self.preMonthView showWeekend:NO];
        [self.nextMonthView showWeekend:NO];
...
- (void)updateMonthViewBegainDate
{
    ...
    else
    {
        ...
        NSLog(@"开始时间===%@",[lastDay bxh_stringWithFormate:@"dd"]);
        self.midMonthView.beaginDate = lastDay;
        self.preMonthView.beaginDate = [self.midMonthView.beaginDate bxh_dateByMinusWeeks:1];
        self.nextMonthView.beaginDate = [self.midMonthView.beaginDate bxh_dateByAddingWeeks:1];
    }
...
- (void)scrollToRight:(BOOL)right
{
    ...
    if (right)
    {
        preFrame.origin.x = self.isShowWeekend?ITEM_WIDTH:ITEM_WIDTH5;
    ...
    else
    {
        preFrame.origin.x = self.bounds.size.width;
        midFrame.origin.x = -self.bounds.size.width;
        nextFrame.origin.x = self.isShowWeekend?ITEM_WIDTH:ITEM_WIDTH5;;
        [UIView animateWithDuration:CalendarScrollTime animations:^{
            self.midMonthView.frame = midFrame;
            self.nextMonthView.frame = nextFrame;
        } completion:^(BOOL finished) {
...
- (BXHCalendarMonthView *)preMonthView
{
    if (!_preMonthView)
    {
        _preMonthView = [[BXHCalendarMonthView alloc] initWithCalendarLayout:[[BXHCalendarLayout alloc] init] isShowWeekend:YES];
...
- (BXHCalendarMonthView *)midMonthView
{
    if (!_midMonthView)
    {
        _midMonthView = [[BXHCalendarMonthView alloc] initWithCalendarLayout:[[BXHCalendarLayout alloc] init] isShowWeekend:YES];
...
- (BXHCalendarMonthView *)nextMonthView
{
    if (!_nextMonthView)
    {
        _nextMonthView = [[BXHCalendarMonthView alloc] initWithCalendarLayout:[[BXHCalendarLayout alloc] init] isShowWeekend:YES];
...
- (BXHWeekendView *)weekendView
{
    if (!_weekendView)
    {
        _weekendView = [[BXHWeekendView alloc] initWithItemWidth:ITEM_WIDTH showWeekend:YES];
